...
class TreeTraversalBottomUp(object):
    def __init__(self):
        self.initial_rule_base = None
        self.transformed_input = None
        self.activation_weight = None
        self.belief_update = None
        self.consequence_val = None
        self.crisp_val = None
    ...
    def traverse_tree(self, obj_list):
        ...
        while len(obj_list):
            ...
            if len(visiting) == len(obj_list):
                ...
                self.initial_rule_base = rule_base.create_rule_base()
                self.transformed_input = rule_base.input_transformation()
                self.activation_weight = rule_base.activation_weight()
                self.belief_update = rule_base.belief_update()
                self.consequence_val = rule_base.aggregate_rule()
                result.insert(count, self.consequence_val)
                parent.consequence_val = self.consequence_val

                crisp_val = 0.0
                for i in range(len(parent.ref_val)):
                    crisp_val += float(parent.ref_val[i]) * float(self.consequence_val[i])
